...
public class IdempotencyService {
    ...
    public TransferResponse findResponse(String key, Object request) {
        String response = String.valueOf(repo.findById(key)
                .filter(r -> r.getRequestHash().equals(request))
                .map(TransferIdempotency::getResponseBody));
        TransferResponse transferResponse;

        try {
            transferResponse = mapper.readValue(response, TransferResponse.class);
